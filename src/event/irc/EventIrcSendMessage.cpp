...
EventIrcSendMessage::EventIrcSendMessage(size_t userId, size_t serverId, const std::string& channel, const std::string& message, IrcMessageType type)
    :
    userId{userId},
    serverId{serverId},
    channel{channel},
...
IrcMessageType EventIrcSendMessage::getType() const {
    return type;
}
